/**
 * @file spi_flash_single_test.cpp
 *
 * @author FTDI
 * @date 2014-07-01
 *
...
inline bool ReadDataBytesCmd(FT_HANDLE ftHandle, uint32 _addr, unsigned char * pData , uint16 size)
{
    uint16 sizeTransferred;
    FT4222_STATUS ftStatus;
    std::vector<unsigned char> tmp;
    tmp.push_back(CmdFastSingleRead);
    tmp.push_back((unsigned char)((_addr & 0xFF0000) >> 16));
    tmp.push_back((unsigned char)((_addr & 0x00FF00) >> 8));
    tmp.push_back((unsigned char)( _addr & 0x0000FF));
    tmp.push_back(0xFF);
...
int main(int argc, char const *argv[])
{
    ...
    if(!ReadDataBytesCmd(ftHandle, startAddr,&recvData[0], recvData.size()))
    {
