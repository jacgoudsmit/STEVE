/**
 * @file spi_slave_test_no_protocol.cpp
 *
 * @author FTDI
 * @date 2015-01-08
 *
...
int main(int argc, char const *argv[])
{
    ...
    ftStatus = FT4222_SPI_SetDrivingStrength(ftHandle,DS_12MA, DS_16MA, DS_16MA);
    if (FT_OK != ftStatus)
    {
    ...
    //    1. write spi slave , it will cache the data and wait for spi master access
    //    1. read spi slave , it will get data from spi master . 
    ...
    printf("SPI slave write data... \n");
    
    ftStatus = FT4222_SPISlave_Write(ftHandle, sent_data, sizeof(sent_data), &sizeTransferred);
    if (FT_OK != ftStatus)
    {
        printf("SPI slave write error\n");
    }
    ...
    if(FT4222_SPISlave_GetRxStatus(ftHandle, &rxSize) == FT_OK)
    {
        if(rxSize>0)
        {
            ...
            printf("SPI slave read data... \n");

            if(FT4222_SPISlave_Read(ftHandle,pRead_data, rxSize, &sizeTransferred)!= FT_OK)
            {
                printf("SPI slave read error\n");
            }
            free(pRead_data);
        }        
    }
    ...
    {
        printf("SPI slave get status error\n");
    }
